...
export class TermsConditionsService {
  ...
  getZipCodeCityStateByZipCode(zipCode: string = ""): Observable<HttpResponse<any>> {
    if (zipCode != "") {
      return this.httpClient.get(encodeURI(this.zipcodeCityStateURI + zipCode), { observe: 'response' });
    }
    else {
      return this.httpClient.get(encodeURI(this.zipcodeCityStateURI + "all"), { observe: 'response' });
    }
  }
