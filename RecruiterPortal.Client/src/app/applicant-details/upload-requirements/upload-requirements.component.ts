...
export class UploadRequirementsComponent implements OnInit {
  ...
  constructor(private uploadFileService: UploadRequirementsService, private confirmationService: ConfirmationService, private messageService: MessageService) { }
  ...
  getEmpClass() {
    this.uploadFileService.getEmpolymentClass().subscribe(data => {
      if (data.status === 200) {
  ...
  onEmploymentClassChange(value) {
    this.isLoading = true;
    this.uploadFileService.updateEmpolymentClass(value)
      .subscribe(data => {
        if (data.status === 200) {
          this.getEmpClass();
        }
