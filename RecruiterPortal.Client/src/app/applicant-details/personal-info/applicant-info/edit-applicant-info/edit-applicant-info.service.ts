...
export class EditApplicantInfoService {
  ...
  getPositionByPositionName(posotion: string): Observable<HttpResponse<any>> {
    return this.http.get(encodeURI(this.positionURI + posotion), { observe: 'response' });
  }

  getCountryName(countryName: string = ""): Observable<HttpResponse<any>> {
    if (countryName != "") {
      return this.http.get(encodeURI(this.countryNameURI + countryName), { observe: 'response' });
    }
    else {
      return this.http.get(encodeURI(this.countryNameURI + "all"), { observe: 'response' });
    }
  }
}
