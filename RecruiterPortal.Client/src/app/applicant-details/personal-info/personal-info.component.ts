...
export class PersonalInfoComponent implements OnInit {
    ...
    constructor(private router: Router, private editApplicantInfoService: EditApplicantInfoService) {
        this.activeIndex = this.selectedActiveIndex;
    }
    getApplicantName() {
        this.editApplicantInfoService.getApplicantInfo().subscribe(data => {
            if (data.body[0].MiddleName || data.body[0].MiddleName != null) {
                this.applicantName = data.body[0].FirstName + " " + data.body[0].MiddleName + " " + data.body[0].LastName;
            }
            else {
                this.applicantName = data.body[0].FirstName + " " + data.body[0].LastName;
            }
        },
            err => { },
            () => { });
