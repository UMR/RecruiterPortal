...
namespace ApplicantPortalAPI.ResourceServer.Controllers
{
    ...
    public class ApplicantInfoController : CustomControllerBase
    {
        ...
        public IActionResult GetEducationInfo(int applicantId)
        {
            try
            {
                #region For Cache Setting
                //var applicantDbConnStringName = base.GetApplicantDBConnectionStringName();
                //_httpContextAccessor.HttpContext.Session.SetString("ApplicantDBConnStringName", "ApplicantDBConnStringName");
                //Utility.SetApplicantDbConnectionStringName(applicantDbConnStringName);
                #endregion
        ...
        public IActionResult GetZipCodeCityStateByZipCode(string zipCode)
        {
            try
            {
                IEnumerable<LookupZipCode> view_LookUp_ZipCodeList;
                if (zipCode == "all")
                {
                    view_LookUp_ZipCodeList = ZipCodeManager.GetZipCodeCityStateByZipCode("");
                }
                else
                {
                    view_LookUp_ZipCodeList = ZipCodeManager.GetZipCodeCityStateByZipCode(zipCode);
                }

                return Ok(new ResponseModels<IEnumerable<LookupZipCode>>(view_LookUp_ZipCodeList));
            }
            catch (Exception ex)
            {
                //Log.Write(ex);
